...
restore_stock() {
  ...
  for _ in {1..10}; do
    ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAcceptedKeyTypes=+ssh-rsa -o ConnectTimeout=8 \
...
if [[ "$online" == 1 ]]; then
  for _ in {1..12}; do
    if ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAcceptedKeyTypes=+ssh-rsa -o ConnectTimeout=8 \
...
if [[ -n "$payload" && "$project_ssh" == 1 ]]; then
  ...
  ssh -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAcceptedKeyTypes=+ssh-rsa -o ConnectTimeout=12 \
    "$mic" 'cat > /tmp/xpr-rootfs.cpio.gz' < "$payload"
  ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAcceptedKeyTypes=+ssh-rsa -o ConnectTimeout=12 \
  ...
  (
    for _ in {1..24}; do
      ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PubkeyAcceptedKeyTypes=+ssh-rsa -o ConnectTimeout=4 \
