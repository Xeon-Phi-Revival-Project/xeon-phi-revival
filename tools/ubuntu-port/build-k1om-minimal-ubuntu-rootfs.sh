...
for applet in sh ls cat grep sed awk basename dd tar tr find mkdir rm cp mv ln readlink touch chmod chown ps mount umount uname hostname env sort head tail wc date df du sleep true false test printf pwd; do
