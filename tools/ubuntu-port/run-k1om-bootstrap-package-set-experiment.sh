...
ssh -n -o BatchMode=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=6 "\$mic" 'echo stock_ssh_ok; test ! -e /opt/xeon-phi-revival/profile.env && echo profile_absent; test ! -e /var/log/xeon-phi-revival/stage2.log && echo stage2_log_absent; test ! -f /var/lib/dpkg/status && echo dpkg_status_absent; test ! -e /usr/bin/python3 && echo python3_absent; test ! -e /usr/bin/pcietool && echo pcietool_absent; test ! -e /usr/bin/dpkg && echo dpkg_absent; test ! -e /usr/bin/apt-get && echo apt_get_absent; test ! -e /usr/bin/apt-cache && echo apt_cache_absent; test ! -e /etc/profile.d/xeon-phi-revival.sh && echo profiled_absent; cat /proc/1/comm'
EOF
