...
release= sdk_root=/opt/mpss/3.4.10 out="$repo_root/build/xpr-k1om"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --release) release=$2; shift 2 ;;
    --sdk-root) sdk_root=$2; shift 2 ;;
    --out) out=$2; shift 2 ;;
    -h|--help) echo "usage: setup-xpr-k1om-toolkit.sh --release RC6.tar.gz [--sdk-root DIR] [--out DIR]"; exit 0 ;;
...
bash "$repo_root/tools/toolchain/build-xpr-k1om-sysroot.sh" --release "$release" --sdk-root "$sdk_root" --out "$out"
mkdir -p "$out/bin"
