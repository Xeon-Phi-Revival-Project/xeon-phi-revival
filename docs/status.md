...
3. **K1OM toolchain and SDK work**: the initial XPR K1OM Toolkit now compiles and runs dynamic C hello and pthread examples on the validated 5110P baseline; it still requires the historical MPSS SDK compiler and a source-built XPR eglibc stage. A source-built standalone-driver investigation has also proven a narrow dynamic hello path, but a recovered KNC IMCI-capable binutils source is still required before XPR can package or claim a fully standalone toolkit.
