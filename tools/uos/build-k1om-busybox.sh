...
usage() {
  cat >&2 <<'EOF'
usage: build-k1om-busybox.sh --source-dir DIR --config FILE --out FILE [--cross-compile PREFIX] [--source-archive FILE] [--work-root DIR] [--jobs N]
...
source_dir=""; config=""; out=""; source_archive=""; cross_compile="k1om-mpss-linux-"; work_root="${HOME}/xeon-phi-revival-local/busybox-builds"; jobs=2
while [[ $# -gt 0 ]]; do
  case "$1" in
