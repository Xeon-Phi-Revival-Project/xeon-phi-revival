...
The first clean-archive build used different absolute source and output paths,
so its linked artifacts did not reproduce the retained target. The final
correction stages the public source in a disposable directory and uses a
private mount namespace to bind it over the historical Kbuild paths without
changing the preserved host directories.

That clean-archive historical-path rebuild reproduced every asserted artifact:
`compile.h`, initramfs, `vmlinux`, `System.map`, compressed image, and final
`bzImage` `d529aecf0de11e0b4a9a036eb0329d1bb9c907fd6a911ce08a10548c9380d4d8`.
The source build is now byte-identical to the hardware-validated kernel. No
additional kernel boot was required for this reproduction proof.
