...
usage() {
  cat >&2 <<'EOF'
usage: build-k1om-busybox.sh --source-dir DIR --config FILE --out FILE [--source-archive FILE] [--work-root DIR] [--jobs N]
...
source_dir=""; config=""; out=""; source_archive=""; work_root="${HOME}/xeon-phi-revival-local/busybox-builds"; jobs=2
while [[ $# -gt 0 ]]; do
  case "$1" in
...
(
  cd "$build_dir"
  make ARCH=k1om CROSS_COMPILE=k1om-mpss-linux- oldconfig > "$run_dir/oldconfig.log" 2>&1
  make -j"$jobs" ARCH=k1om CROSS_COMPILE=k1om-mpss-linux- > "$run_dir/build.log" 2>&1
)
readelf -h "$build_dir/busybox" | grep -q 'Machine:.*Intel K1OM' || { echo 'BusyBox is not K1OM' >&2; exit 11; }
readelf -d "$build_dir/busybox" 2>/dev/null | grep -q NEEDED && { echo 'BusyBox is not static' >&2; exit 12; } || true
cp -a "$build_dir/busybox" "$out"
{
  ...
  printf 'compiler=%s\n' "$(k1om-mpss-linux-gcc --version | head -n1)"
