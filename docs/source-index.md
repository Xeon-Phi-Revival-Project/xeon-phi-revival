...
| [Ubuntu Noble OpenSSL source package](https://packages.ubuntu.com/source/noble/openssl) | Ubuntu 24.04 source baseline used for K1OM OpenSSL / Python `_ssl` and `_hashlib` work. | Use Ubuntu source package terms; keep generated K1OM artifacts private unless licensing and payload provenance are clear. |
| [Ubuntu Noble libffi source package](https://packages.ubuntu.com/source/noble/libffi) | Ubuntu 24.04 source baseline used for the passing K1OM libffi / `_ctypes` port. | Use Ubuntu source package terms; keep generated K1OM artifacts private unless licensing and payload provenance are clear. |
