...
kernel_license_member="$(tar -tzf "$work/kernel-source.tar.gz" | awk '/\/phi-kernel\/COPYING$/ && !found { print; found=1 } END { exit !found }')"
...
tar -xOjf "$module_source" "$(tar -tjf "$module_source" | awk '/(^|\/)COPYING$/ && !found { print; found=1 } END { exit !found }')" > "$binary_root/LICENSES/MPSS-modules-GPL-2.0-only.txt"
tar -xOjf "$busybox_source" "$(tar -tjf "$busybox_source" | awk '/(^|\/)LICENSE$/ && !found { print; found=1 } END { exit !found }')" > "$binary_root/LICENSES/BusyBox-1.19.4-GPL-2.0-only.txt"
tar -xOJf "$eglibc_orig" "$(tar -tJf "$eglibc_orig" | awk '/(^|\/)COPYING\.LIB$/ && !found { print; found=1 } END { exit !found }')" > "$binary_root/LICENSES/LGPL-2.1-or-later.txt"
