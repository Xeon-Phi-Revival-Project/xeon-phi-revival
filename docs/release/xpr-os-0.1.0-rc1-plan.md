...
an optional payload, uploads it by SCP after bootstrap SSH, invokes staging,
and waits for the RC PID 1 marker before its existing rollback trap.

Static validation confirms payload gzip/newc integrity and required paths. The
remaining validation is one bounded hardware test of transfer, checksum,
staging, PID 1 `switch_root`, RC SSH, smoke tests, and rollback.
